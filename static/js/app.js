...
async function searchLocation( query, type = 'birth' ) {
    if ( !query || query.length < 3 ) {
        const resultsDiv = document.getElementById( type === 'birth' ? 'locationResults' : 'transit_location_suggestions' );
    ...
    try {
        const response = await fetch( `/search_location?query=${encodeURIComponent( query )}` );
...
function selectLocation( location, type = 'birth' ) {
    if ( type === 'birth' ) {
        document.getElementById( 'birth_place' ).value = `${location.name}, ${location.country}`;
        ...
        document.getElementById( 'transit_location_search' ).value = `${location.name}, ${location.country}`;
        document.getElementById( 'transit_latitude' ).value = location.lat;
        document.getElementById( 'transit_longitude' ).value = location.lon;
        document.getElementById( 'transit_location_suggestions' ).style.display = 'none';
    }
}
...
async function submitAstroForm( event ) {
    ...
    try {
        ...
        const transitLongitude = document.getElementById( 'transit_longitude' )?.value;
        ...
        if ( result.success ) {
            try {
                ...
                alert( "Veriler tarayıcınıza kaydedilemedi. Yine de sonuçlar gösterilecektir." );
...
window.astro = {
    ...
    async showChatInterface() {
        try {
            const interpretationText = document.getElementById( 'interpretationText' );
            const chatInterface = document.getElementById( 'chatInterface' );
            const lastInterpretationDiv = document.getElementById( 'lastInterpretation' );
...
document.addEventListener( 'DOMContentLoaded', function () {
    ...
    if ( document.getElementById( 'closeModalBtn' ) ) {
        document.getElementById( 'closeModalBtn' ).addEventListener( 'click', () => window.astro.closeModal() );
    }
    ...
    if ( playButton ) {
        playButton.addEventListener( 'click', () => {
            if ( window.speechSynthesis.paused ) {
                ...
                window.astro.startSpeech( document.getElementById( 'interpretationText' ).textContent );
            }
        } );
    }
    ...
    if ( document.getElementById( 'askQuestionBtn' ) ) {
        document.getElementById( 'askQuestionBtn' ).addEventListener( 'click', () => window.astro.showChatInterface() );
    }
    ...
    // Doğum yeri arama event listener
    ...
    // Transit yeri arama event listener
...
document.addEventListener( 'click', function ( e ) {
    ...
    if ( locationResults && !e.target.closest( '#birth_place' ) && !e.target.closest( '#locationResults' ) ) {
...
function getPlanetSymbol( planetName ) {
    const symbols = {
        ...
        "Chiron": " Chiron", "Lilith": " Lilith", // İkonları yoksa isimleri
...
function displayNatalData( astroData ) {
    ...
    if ( astroData.birth_info ) {
        if ( birthDatetimeEl ) birthDatetimeEl.textContent = `${astroData.birth_info.date || ''} ${astroData.birth_info.time || ''}`;
        if ( birthLocationEl ) birthLocationEl.textContent = astroData.birth_info.location_name || ( astroData.birth_info.location ? `${astroData.birth_info.location.latitude}, ${astroData.birth_info.location.longitude}` : 'Bilinmiyor' );
    }
    if ( houseSystemEl ) houseSystemEl.textContent = astroData.house_system || 'Bilinmiyor';

    if ( astroData.natal_summary ) {
        if ( ascendantInfoEl ) {
            ascendantInfoEl.textContent = `${astroData.natal_summary.ascendant_sign || ''} ${astroData.natal_summary.ascendant_degree !== undefined ? parseFloat( astroData.natal_summary.ascendant_degree ).toFixed( 2 ) + '°' : ''}`;
        }
        if ( mcInfoEl ) {
            mcInfoEl.textContent = `${astroData.natal_summary.mc_sign || ''} ${astroData.natal_summary.mc_degree !== undefined ? parseFloat( astroData.natal_summary.mc_degree ).toFixed( 2 ) + '°' : ''}`;
        }
    }

    // 2. Natal Yorum Özetleri (natal-summary-interpretations)
    const summaryInterpretationsEl = document.getElementById( 'natal-summary-interpretations' );
    if ( summaryInterpretationsEl ) {
        summaryInterpretationsEl.innerHTML = '<h3 class="text-lg font-semibold mb-4">Doğum Haritası Yorum Özeti</h3>'; // Başlığı koru
        if ( astroData.natal_summary_interpretation && astroData.natal_summary_interpretation.length > 0 ) {
            ...
            astroData.natal_summary_interpretation.forEach( item => {
                ...
                if ( item.includes( 'Element Dağılımı:' ) ) {
                    itemDiv.classList.add( 'bg-gradient-to-r', 'from-slate-800', 'to-blue-900' );
                    iconHtml = '<div class="w-8 h-8 flex items-center justify-center rounded-full bg-blue-500 mr-3"><i class="fas fa-wind text-white"></i></div>';
                ...
                } else if ( item.includes( 'Element:' ) ) { // 'Element Dağılımı'ndan sonra kontrol etmeli
                    itemDiv.classList.add( 'bg-gradient-to-r', 'from-slate-800', 'to-blue-900' );
                    iconHtml = '<div class="w-8 h-8 flex items-center justify-center rounded-full bg-blue-500 mr-3"><i class="fas fa-wind text-white"></i></div>';
                    contentHtml = `<div class="flex items-center">${iconHtml}<p class="text-slate-200">${item}</p></div>`;
                } else if ( item.includes( 'Nitelik:' ) ) {
                    itemDiv.classList.add( 'bg-gradient-to-r', 'from-slate-800', 'to-green-900' );
                    iconHtml = '<div class="w-8 h-8 flex items-center justify-center rounded-full bg-green-500 mr-3"><i class="fas fa-exchange-alt text-white"></i></div>';
                    contentHtml = `<div class="flex items-center">${iconHtml}<p class="text-slate-200">${item}</p></div>`;
                } else if ( item.includes( 'Polarite:' ) ) {
                    itemDiv.classList.add( 'bg-gradient-to-r', 'from-slate-800', 'to-purple-900' );
                    iconHtml = '<div class="w-8 h-8 flex items-center justify-center rounded-full bg-purple-500 mr-3"><i class="fas fa-yin-yang text-white"></i></div>';
                    contentHtml = `<div class="flex items-center">${iconHtml}<p class="text-slate-200">${item}</p></div>`;
                } else if ( item.includes( 'Güneş-Ay Fazı:' ) || item.includes( 'Güneş-Ay İlişkisi:' ) ) {
                    itemDiv.classList.add( 'bg-gradient-to-r', 'from-slate-800', 'to-yellow-900' );
                    iconHtml = '<div class="w-8 h-8 flex items-center justify-center rounded-full bg-yellow-500 mr-3"><i class="fas fa-moon text-white"></i></div>';
                    contentHtml = `<div class="flex items-center">${iconHtml}<p class="text-slate-200">${item}</p></div>`;
                } else if ( item.includes( 'Yükselen Yorumu:' ) ) {
                    itemDiv.classList.add( 'bg-gradient-to-r', 'from-slate-800', 'to-pink-900' );
                    iconHtml = '<div class="w-8 h-8 flex items-center justify-center rounded-full bg-pink-500 mr-3"><i class="fas fa-arrow-up text-white"></i></div>';
                    contentHtml = `<div class="flex items-center">${iconHtml}<p class="text-slate-200">${item}</p></div>`;
                } else if ( item.match( /\d+\.\s*Ev:/i ) || item.startsWith( 'Ev:' ) ) { // "1. Ev:", "Ev: Koç" gibi formatlar
                    itemDiv.classList.add( 'bg-gradient-to-r', 'from-slate-800', 'to-indigo-900' );
                    iconHtml = '<div class="w-8 h-8 flex items-center justify-center rounded-full bg-indigo-500 mr-3"><i class="fas fa-home text-white"></i></div>';

                    const parts = item.split( ':' );
                    const houseNumberText = parts[0].trim();
                    const planetsText = parts.length > 1 ? parts[1].trim() : '';
                    const planets = planetsText ? planetsText.split( ',' ).map( p => p.trim() ).filter( p => p ) : [];

                    let planetsHtml = '<div class="ml-11 flex flex-wrap gap-2 mt-1">';
                    if ( planets.length > 0 ) {
                        planets.forEach( planet => {
                            let bgColor = 'bg-slate-700 text-slate-200'; // Default
                            if ( planet.toLowerCase().includes( 'güneş' ) || planet.toLowerCase().includes( 'sun' ) ) bgColor = 'bg-yellow-700 text-yellow-200';
                            else if ( planet.toLowerCase().includes( 'ay' ) || planet.toLowerCase().includes( 'moon' ) ) bgColor = 'bg-blue-700 text-blue-200';
                            else if ( planet.toLowerCase().includes( 'merkür' ) || planet.toLowerCase().includes( 'mercury' ) ) bgColor = 'bg-purple-700 text-purple-200';
                            else if ( planet.toLowerCase().includes( 'venüs' ) || planet.toLowerCase().includes( 'venus' ) ) bgColor = 'bg-pink-700 text-pink-200';
                            else if ( planet.toLowerCase().includes( 'mars' ) ) bgColor = 'bg-red-700 text-red-200';
                            else if ( planet.toLowerCase().includes( 'jüpiter' ) || planet.toLowerCase().includes( 'jupiter' ) ) bgColor = 'bg-orange-700 text-orange-200';
                            else if ( planet.toLowerCase().includes( 'satürn' ) || planet.toLowerCase().includes( 'saturn' ) ) bgColor = 'bg-gray-700 text-gray-200';
                            // Diğer gezegenler eklenebilir
                            planetsHtml += `<span class="px-2 py-1 text-xs rounded-full ${bgColor}">${planet}</span>`;
                        } );
                    } else {
                        planetsHtml += '<span class="text-xs text-slate-400">Bu evde gezegen yok veya belirtilmemiş.</span>';
                    }
                    planetsHtml += '</div>';
                ...
                } else { // Diğer genel yorumlar
                    itemDiv.classList.add( 'border', 'border-slate-700' );
                    if ( item.length < 50 ) { // Kısa genel yorumlar için ikon
                        iconHtml = '<div class="w-8 h-8 flex items-center justify-center rounded-full bg-slate-500 mr-3"><i class="fas fa-info-circle text-white"></i></div>';
            ...
            summaryInterpretationsEl.innerHTML += '<p class="text-slate-400">Yorum özeti bulunamadı.</p>';
        }
    }
    ...
    if ( mainPlanetsListEl ) {
        mainPlanetsListEl.innerHTML = '<h3 class="text-lg font-semibold mb-4">Temel Pozisyonlar</h3>'; // Başlığı koru
        ...
        const createPlanetElement = ( name, data, isPoint = false ) => {
            ...
            if ( planetKeyLower.includes( 'sun' ) || planetKeyLower.includes( 'güneş' ) ) {
                iconClass = 'fas fa-sun'; bgColor = 'bg-yellow-500';
            } else if ( planetKeyLower.includes( 'moon' ) || planetKeyLower.includes( 'ay' ) ) {
                iconClass = 'fas fa-moon'; bgColor = 'bg-blue-400';
            } else if ( planetKeyLower.includes( 'mercury' ) || planetKeyLower.includes( 'merkür' ) ) {
                iconClass = 'fas fa-brain'; bgColor = 'bg-purple-400';
            } else if ( planetKeyLower.includes( 'venus' ) || planetKeyLower.includes( 'venüs' ) ) {
                iconClass = 'fas fa-venus'; bgColor = 'bg-pink-400';
            } else if ( planetKeyLower.includes( 'mars' ) ) {
                iconClass = 'fas fa-mars'; bgColor = 'bg-red-500';
            } else if ( planetKeyLower.includes( 'jupiter' ) || planetKeyLower.includes( 'jüpiter' ) ) {
                iconClass = 'fas fa-jupiter'; bgColor = 'bg-orange-500';
            } else if ( planetKeyLower.includes( 'saturn' ) || planetKeyLower.includes( 'satürn' ) ) {
                iconClass = 'fas fa-saturn'; bgColor = 'bg-yellow-700';
            } else if ( planetKeyLower.includes( 'uranus' ) || planetKeyLower.includes( 'uranüs' ) ) {
                iconClass = 'fas fa-uranus'; bgColor = 'bg-sky-500';
            } else if ( planetKeyLower.includes( 'neptune' ) || planetKeyLower.includes( 'neptün' ) ) {
                iconClass = 'fas fa-neptune'; bgColor = 'bg-blue-600';
            } else if ( planetKeyLower.includes( 'pluto' ) || planetKeyLower.includes( 'plüton' ) ) {
                iconClass = 'fas fa-meteor'; bgColor = 'bg-indigo-600';
            } else if ( planetKeyLower.includes( 'ascendant' ) || planetKeyLower.includes( 'yükselen' ) || name.toUpperCase() === 'ASC' ) {
                iconClass = 'fas fa-arrow-alt-circle-up'; bgColor = 'bg-green-500';
            } else if ( planetKeyLower.includes( 'midheaven' ) || name.toUpperCase() === 'MC' ) {
                iconClass = 'fas fa-crosshairs'; bgColor = 'bg-cyan-500';
            }


            const sign = data.sign || '';
            const degree = data.degree !== undefined ? parseFloat( data.degree ).toFixed( 2 ) + '°' : '';
            const house = data.house ? `${data.house}. Evde` : ( isPoint ? '' : 'Ev bilgisi yok' );
        ...
        if ( astroData.natal_planet_positions ) {
            ...
            planetOrder.forEach( planetName => {
                if ( astroData.natal_planet_positions[planetName] ) {
                    planetsContainer.appendChild( createPlanetElement( planetName, astroData.natal_planet_positions[planetName] ) );
                }
            } );
        }
        ...
        if ( astroData.natal_summary ) {
            if ( astroData.natal_summary.ascendant_sign ) {
                planetsContainer.appendChild( createPlanetElement( 'Yükselen (ASC)', {
            ...
            if ( astroData.natal_summary.mc_sign ) {
                planetsContainer.appendChild( createPlanetElement( 'MC (Tepe Noktası)', {
        ...
        if ( planetsContainer.hasChildNodes() ) {
            ...
            mainPlanetsListEl.innerHTML += '<p class="text-slate-400">Gezegen pozisyonları bulunamadı.</p>';
        }
...
window.onload = function () {
    ...
    console.log( "LocalStorage'dan astro_data_str (ilk 100 char):", astroDataStr ? astroDataStr.substring( 0, 100 ) + "..." : "yok" ); // DEBUG
    ...
    try {
        if ( astroDataStr ) {
            ...
            console.log( "LocalStorage'dan astro_data yüklendi (JSON.stringify ile ilk 200 char):", window.astroData ? JSON.stringify( window.astroData ).substring( 0, 200 ) + "..." : "parse edilemedi" );
        } else {
        ...
        console.log( "LocalStorage'dan birthPlaceName yüklendi:", window.birthPlaceName );
    ...
    if ( window.astroData ) {
        ...
        if ( birthDateEl && window.astroData.birth_info && window.astroData.birth_info.datetime ) {
            const [datePart, timePart] = window.astroData.birth_info.datetime.split( ' ' );
            birthDateEl.textContent = `${datePart || ''} ${timePart || ''}`;
        } else if ( birthDateEl ) {
            birthDateEl.textContent = 'Bilinmiyor';
        }

        if ( positionsEl ) positionsEl.textContent = ( window.astroData.natal_planet_positions && Object.keys( window.astroData.natal_planet_positions ).length > 0 ) ? 'Mevcut' : 'Yok';
        if ( transitDateEl && window.astroData.transit_date ) transitDateEl.textContent = window.astroData.transit_date;
        ...
        if ( birthInfoSummaryDiv ) {
            ...
            const birthDatetimeSummaryEl = document.getElementById( 'birth-datetime-summary' );
            const birthLocationSummaryEl = document.getElementById( 'birth-location-summary' );
            ...
            const ascendantSignSummaryEl = document.getElementById( 'ascendant-sign-summary' );
            ...
            if ( window.astroData.birth_info ) {
                if ( birthDatetimeSummaryEl && window.astroData.birth_info.datetime ) {
                    const [datePart, timePart] = window.astroData.birth_info.datetime.split( ' ' );
                    birthDatetimeSummaryEl.textContent = `${datePart || ''} ${timePart || ''}`;
                    showSummary = true;
                ...
                if ( birthLocationSummaryEl ) {
                    birthLocationSummaryEl.textContent = window.birthPlaceName || 'Bilinmiyor';
            ...
            if ( window.astroData.natal_planet_positions ) {
                if ( sunSignSummaryEl && window.astroData.natal_planet_positions.Sun ) {
                    sunSignSummaryEl.textContent = window.astroData.natal_planet_positions.Sun.sign || '-';
                    if ( window.astroData.natal_planet_positions.Sun.sign ) showSummary = true;
                } else if ( sunSignSummaryEl ) {
                    sunSignSummaryEl.textContent = '-';
                }
                if ( moonSignSummaryEl && window.astroData.natal_planet_positions.Moon ) {
                    moonSignSummaryEl.textContent = window.astroData.natal_planet_positions.Moon.sign || '-';
                    if ( window.astroData.natal_planet_positions.Moon.sign ) showSummary = true;
            ...
            if ( window.astroData.natal_summary ) {
                if ( ascendantSignSummaryEl ) {
                    ascendantSignSummaryEl.textContent = window.astroData.natal_summary.ascendant_sign || '-';
        ...
        console.warn( "window.astroData yüklenemediği için displayNatalData çağrılmayacak ve debug/header bilgileri güncellenmeyecek." ); // DEBUG
        ...
        if ( document.getElementById( 'natal-birth-datetime' ) ) document.getElementById( 'natal-birth-datetime' ).textContent = 'Veri hesaplanmadı.';
        if ( document.getElementById( 'natal-birth-location' ) ) document.getElementById( 'natal-birth-location' ).textContent = 'Veri hesaplanmadı.';
        if ( document.getElementById( 'natal-house-system' ) ) document.getElementById( 'natal-house-system' ).textContent = 'Veri hesaplanmadı.';
        if ( document.getElementById( 'natal-ascendant-info' ) ) document.getElementById( 'natal-ascendant-info' ).textContent = 'Veri hesaplanmadı.';
        if ( document.getElementById( 'natal-mc-info' ) ) document.getElementById( 'natal-mc-info' ).textContent = 'Veri hesaplanmadı.';
        if ( document.getElementById( 'natal-summary-interpretations' ) ) document.getElementById( 'natal-summary-interpretations' ).innerHTML = '<h3 class="text-lg font-semibold mb-4">Doğum Haritası Yorum Özeti</h3><p class="text-slate-400">Yorum özeti verisi bulunamadı.</p>';
        if ( document.getElementById( 'natal-main-planets-list' ) ) document.getElementById( 'natal-main-planets-list' ).innerHTML = '<h3 class="text-lg font-semibold mb-4">Temel Pozisyonlar</h3><p class="text-slate-400">Gezegen pozisyonları verisi bulunamadı.</p>';
    }
