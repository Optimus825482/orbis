...
def init_cache(app):
    ...
    if cache_type == "redis":
        ...
        if redis_password:
            cache_config['CACHE_REDIS_URL'] = f"redis://:{redis_password}@{redis_host}:{redis_port}/{redis_db}"
        else:
            cache_config['CACHE_REDIS_URL'] = f"redis://{redis_host}:{redis_port}/{redis_db}"
        ...
        def wrapper(*args, **kwargs):
            ...
            for k in sorted(kwargs.keys()):
                ...
                if isinstance(v, (str, int, float, bool)):
                    ...
                    key_parts.append(f"{k}:{hashlib.md5(sorted_dict.encode()).hexdigest()[:8]}")
...
def cached_location_search(timeout: int = Constants.CACHE_TTL_LOCATION_SEARCH):
    ...
    def decorator(f: Callable) -> Callable:
        ...
        def wrapper(query: str, *args, **kwargs):
            ...
            # Cache key oluştur
            cache_key = f"location_search:{hashlib.md5(normalized_query.encode()).hexdigest()}"
...
def cached_astro_calculation(timeout: int = Constants.CACHE_TTL_ASTRO_CALCULATION):
    ...
    def decorator(f: Callable) -> Callable:
        @wraps(f)
        def wrapper(birth_date: Any, birth_time: Any, latitude: float, longitude: float, *args, **kwargs):
            # Cache key oluştur
            key_parts = [
                "astro_calc",
                str(birth_date),
...
def get_cache_stats() -> dict:
    ...
    try:
        ...
        if redis_client:
            ...
            return {
                'type': 'simple',
                'message': 'Stats not available for simple cache'
            }
...
# =============================================================================
def cached_session_data(session_id: str, timeout: int = Constants.SESSION_TIMEOUT_SECONDS):
    """
...
def set_session_cache(session_id: str, data: Any, timeout: int = Constants.SESSION_TIMEOUT_SECONDS):
    """
